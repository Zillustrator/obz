...
TEST_CASE("mpsc_ring_queue destroys remaining published values on destruction") {
    struct tracked {
        explicit tracked(int value, int& destroyed_count) noexcept
            : value(value),
    ...
    {
        obz::mpsc_ring_queue<tracked, 3> queue;
